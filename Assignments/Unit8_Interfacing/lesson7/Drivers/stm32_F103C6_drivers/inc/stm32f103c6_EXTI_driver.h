...
typedef struct
{
	EXTI_GPIO_Mapping_t  			EXTI_PIN; 			// specifies the External interrupt Gpio mapping
														// This Paramter must be based on @ref EXTI_define
	uint8_t                         Trigger_Case ;	   //specifies RISING or FALLING or both TRIgGer.
													 //This parameter must be specified on @ref EXTI_Trigger_define
	uint8_t                         IRQ_EN       ;   // Enable or Disable the EXTI IRQ ( That will Enable The IRQ MASK on the EXTI and also on the NVIC interrupt  )
													// This parameter must be set based on @ref EXTI_IRQ_define
	void(*p_IRQ_CallBack)(void);					// Set the function which will be called once the IRQ habben
